...
import { campusForHash, hashForCampus } from "./campus-routes.mjs";
...
function shuffleSpots(spots: readonly ArtSpot[]) {
...
function shuffleNames(spots: readonly ArtSpot[]) {
...
function shuffleChoiceNames(spots: readonly ArtSpot[]) {
  const names = shuffleNames(spots);
  const originalOrder = spots.map((spot) => spot.name);
  if (names.length > 1 && names.every((name, index) => name === originalOrder[index])) {
    [names[0], names[1]] = [names[1], names[0]];
  }
...
function sampleArtSpots(spots: readonly ArtSpot[], count = 4) {
