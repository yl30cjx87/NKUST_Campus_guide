...
test("home loads a native, disposable 3D map with clickable campus models", async () => {
  const component = await readFile(new URL("../app/campus-map.tsx", import.meta.url), "utf8");
...
test("keeps the 2D art map data wired into the game source", async () => {
  ...
  assert.match(page, /function shuffleSpots/);
  assert.match(page, /function shuffleChoiceNames/);
